...
#ifndef MATRIX_TEMPLATE_LIBRARY_CPP_STATIC_COL_MATRIX_H_
#define MATRIX_TEMPLATE_LIBRARY_CPP_STATIC_COL_MATRIX_H_

#include <matrix/static_matrix.h>

namespace mtl {
template<typename T, std::size_t Cols>
class static_col_matrix : public static_matrix<T, 1, Cols> {
public:
  using value_type = T;
...
public:
  explicit MATRIX_CXX17_CONSTEXPR static_col_matrix(value_type fill = value_type{})
	  : static_matrix<T, 1, Cols>(fill) {};

  template<typename Container>
  explicit MATRIX_CXX17_CONSTEXPR static_col_matrix(const Container &container)
	  : static_matrix<T, 1, Cols>(container) {};
...
#endif //MATRIX_TEMPLATE_LIBRARY_CPP_STATIC_COL_MATRIX_H_
